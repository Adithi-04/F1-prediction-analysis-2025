...
session_2024.load()
laps_2024 = session_2024.laps[["Driver", "LapTime", "Sector1Time", "Sector3Time"]].copy()
laps_2024.dropna(inplace=True)

for col in ["LapTime", "Sector1Time", "Sector3Time"]:
    laps_2024[f"{col} (s)"] = laps_2024[col].dt.total_seconds()

# Average sector times per driver
sector_times_2024 = laps_2024.groupby("Driver")[["Sector1Time (s)", "Sector3Time (s)"]].mean().reset_index()
...
X = merged_data[[
    "QualifyingTime (s)", 
    "Sector1Time (s)", "Sector3Time (s)", 
    "WetPerformanceFactor", 
    # "SeasonPoints"
]].fillna(0)

# Amplify importance of QualifyingTime and SeasonPoints
X["QualifyingTime (s)"] *= 2.5   # Amplify Qualifying Time 2.5x
# X["SeasonPoints"] *= 3.0         # Amplify Season Points 3x
...
# Train model
model = GradientBoostingRegressor(n_estimators=300, learning_rate=0.05, max_depth=4, random_state=38)
